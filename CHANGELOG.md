...
- v1.1.1（已发行）- 2026.7.31
  v1.1.1，播放详情页与迷你小窗引入封面取色毛玻璃背景，重构歌词滚动对齐逻辑，修复 seek 进度错位等多项体验问题。

  新增：
  - 封面主色调提取：MusicManager 新增 currentCoverColor 属性，基于颜色直方图算法（4-bit 量化、跳过过暗/过亮/过灰像素）从当前封面提取主色调，切歌时自动更新暴露给 QML
  - 播放详情页封面取色 + 毛玻璃背景：四层叠加结构（兜底深色 → 放大封面 + MultiEffect 毛玻璃模糊纹理 → 主色调半透明染色 → 深色压暗保证可读），切歌时颜色 600ms 平滑过渡
  - 迷你小窗封面取色 + 毛玻璃背景：与详情页同款风格，每层自带 radius:8 圆角，MultiEffect 同时启用 blur 与 mask
